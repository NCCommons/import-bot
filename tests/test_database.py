...
class TestDatabase:
    ...
    def test_record_upload_with_unicode_filename(self, temp_db):
        ...
        with temp_db._get_connection() as conn:
            result = conn.execute(
                "SELECT * FROM uploads WHERE filename='测试文件.jpg'"
            ).fetchone()
    ...
    def test_record_upload_with_long_error_message(self, temp_db):
        ...
        with temp_db._get_connection() as conn:
            result = conn.execute(
                "SELECT error FROM uploads WHERE filename='fail.jpg'"
            ).fetchone()
    ...
    def test_record_page_processing_with_zero_values(self, temp_db):
        ...
        with temp_db._get_connection() as conn:
            result = conn.execute(
                "SELECT * FROM pages WHERE page_title='Empty Page'"
            ).fetchone()
    ...
    def test_record_page_processing_with_unicode_title(self, temp_db):
        ...
        with temp_db._get_connection() as conn:
            result = conn.execute(
                "SELECT * FROM pages WHERE page_title='页面标题'"
            ).fetchone()
    ...
    def test_record_page_upsert_behavior(self, temp_db):
        """Test that recording same page twice updates record."""
        temp_db.record_page_processing('Test Page', 'en', 5, 3)
        temp_db.record_page_processing('Test Page', 'en', 8, 5)
        ...
        with temp_db._get_connection() as conn:
            ...
            # Should only have one record (updated)
            assert len(results) == 1
            assert results[0]['templates_found'] == 8
            assert results[0]['files_uploaded'] == 5